...
The reason why this is done is because if you notice while using 
s aim assist, you're free to move your pitch up and down when targetting a player,
...
void AimAssist::RenderUpdate()
{
	...
	if (fovCircle) {
		...
		// Definiere den Mittelpunkt des Kreises
		ImVec2 center(screenSize.x / 2, screenSize.y / 2);

		// Zeitabhängiger Winkel für RGB-Animation
		static float angle = 0.0f;
		angle += ImGui::GetIO().DeltaTime * 1.5f; // Geschwindigkeit der Drehung
		if (angle > 2 * PI) angle -= 2 * PI;

		// Berechne die RGB-Farbe basierend auf dem Winkel (für eine rotierende Animation)
		ImColor rgbColor(
			(int)(128 + 127 * sinf(angle)),      // R-Wert
			(int)(128 + 127 * sinf(angle + 2.0f)), // G-Wert
			(int)(128 + 127 * sinf(angle + 4.0f)), // B-Wert
			255  // Alpha-Wert (vollständig sichtbar)
		);

		// Zeichne den Kreis mit dickeren Linien und RGB-Farbe
		// Verwende den Farbwert rgbColor, der sich mit dem Winkel ändert
		//ImGui::GetWindowDrawList()->AddCircle(center, circleRadius + 1.5f, ImColor(0, 0, 0, 150), 64, 4.0f); // Schwarzer Rand
		ImGui::GetWindowDrawList()->AddCircle(center, circleRadius, rgbColor, 64, 2.5f);  // Hauptkreis mit RGB-Farbe
		//ImGui::GetWindowDrawList()->AddCircle(center, circleRadius - 1.5f, ImColor(0, 0, 0, 100), 64, 4.0f); // Innerer Rand
	}
